...
void main() {
  ...
  for (var k = 0; k < listOfString.length; k = k + 3) {
    ...
    for (var i = 0; i < 100; i++) {
      for (var j = 0; j < 100; j++) {
        var ai = int.parse(match.elementAt(0).group(0)!);
        var bi = int.parse(match2.elementAt(0).group(0)!);
        var ci = int.parse(match3.elementAt(0).group(0)!);
        var aj = int.parse(match.elementAt(1).group(0)!);
        var bj = int.parse(match2.elementAt(1).group(0)!);
        var cj = int.parse(match3.elementAt(1).group(0)!);
        if (ai * i + bi * j == ci && aj * i + bj * j == cj) {
          answers.add(i * 3 + j);
        }
      }
    }
